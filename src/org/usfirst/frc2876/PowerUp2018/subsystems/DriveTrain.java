...
public class DriveTrain extends Subsystem {
	...
	public DriveTrain() {
		// This next line didn't work. The wheels keep moving.
	...
	public void setVelocityMode(boolean velocity) {
		if (velocity) {
	...
	public boolean isVelocityMode() {
		if (rightMaster.getMode() == ControlMode.Velocity &&
				leftMaster.getMode() == ControlMode.Velocity) {
			return true;
		} else if ((rightMaster.getMode() == ControlMode.Velocity &&
				leftMaster.getMode() != ControlMode.Velocity) 
				|| (rightMaster.getMode() != ControlMode.Velocity &&
				leftMaster.getMode() == ControlMode.Velocity) ) {
			// TODO Throw an exception.. should never have talons be in different modes. I don't know syntax to throw exception
			// throw new Exception("Drive talons are in different modes");
		}
		return false;
	}
	...
	private double stickToNativeVelocity(double stick) {
		return stick * 500 * 4096 / 600;
	}
	
	public void arcadeDrive(double xSpeed, double zRotation) {
		// TODO: If we are in control mode we need to convert speed and rotation
		// into rpm, native units? Joysticks give -1 to 1 range.
		// See https://github.com/CrossTheRoadElec/Phoenix-Examples-Languages/blob/master/Java/VelocityClosedLoop/src/org/usfirst/frc/team217/robot/Robot.java#L68
		if (isVelocityMode()) {
			double x = stickToNativeVelocity(xSpeed);
			double z = stickToNativeVelocity(zRotation);
	...
	public void tankDrive(double leftSpeed, double rightSpeed) {
		// TODO: see comment in arcadeDrive about converting if in velocity mode
		if (isVelocityMode()) {
			double l = stickToNativeVelocity(leftSpeed);
			double r = stickToNativeVelocity(rightSpeed);
	...
	public void updateSmartDashboard() {
		...
		// SmartDashboard.putBoolean("is navx connecticut?",
		// navx.isConnected());
